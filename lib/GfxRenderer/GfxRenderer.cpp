...
bool GfxRenderer::getGlyphIs2Bit(const EpdFontData* fontData, const EpdGlyph* glyph) const {
  // A CJK fallback SdCardFont back-fills glyphs into a UI font's EpdFontData via
  // glyphMissHandler. Those overflow glyphs carry the fallback font's bitmap
  // depth, which can differ from the host UI font's (builtin UI fonts are 1-bit;
  // LXGW/CJK .cpfont are 2-bit). Decode overflow glyphs with the fallback depth,
  // everything else with the host font's own flag.
  if (fontData->glyphMissCtx) {
    auto* sdFont = SdCardFont::fromMissCtx(fontData->glyphMissCtx);
    if (sdFont->isOverflowGlyph(glyph)) {
      return sdFont->is2Bit();
    }
  }
  return fontData->is2Bit;
}
...
static void renderCharScaled(const GfxRenderer& renderer, GfxRenderer::RenderMode renderMode,
  ...
  if (renderer.getGlyphIs2Bit(fontData, glyph)) {
...
static void renderCharImpl(const GfxRenderer& renderer, GfxRenderer::RenderMode renderMode,
  ...
  const EpdFontData* fontData = fontFamily.getData(style);
  const bool is2Bit = renderer.getGlyphIs2Bit(fontData, glyph);
