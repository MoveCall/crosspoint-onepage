...
#ifdef ONEPAGE_C61
  // OnePage's four front buttons cluster physically as [Back] [Left Right] [Confirm]
  // (see hardware_io.md: a narrow key, a wide 2-key middle, a narrow key). Pin Back
  // and Confirm to the edges and tuck Left/Right together in the middle so each hint
  // sits over its real button. Label order comes from MappedInputManager::mapLabels
  // (Back, Left, Right, Confirm on OnePage). 480px-wide portrait, 80px buttons.
  constexpr int onepageButtonPositions[] = {20, 150, 250, 380};
  const int* buttonPositions = onepageButtonPositions;
#else
