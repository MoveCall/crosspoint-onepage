...
#ifdef ONEPAGE_C61
  // OnePage's four front buttons are physically laid out (left->right) as
  // Back, Left, Right, Confirm (see hardware_io.md: K1 返回 / K2 左 / K3 右 /
  // K4 确认). drawButtonHints paints btn1..btn4 in that same left->right order,
  // so emit the labels in physical order here. The X4 default below assumes a
  // Back, Confirm, Left, Right layout, which would land 3 of the 4 hints over
  // the wrong buttons on OnePage.
