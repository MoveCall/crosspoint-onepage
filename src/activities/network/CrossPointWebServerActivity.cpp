...
#include <DNSServer.h>
#ifdef ONEPAGE_C61
// OnePage: espressif/mdns isn't in the slim C61 source-build component set
// (ESPmDNS.h compiles to empty without CONFIG_MDNS_MAX_INTERFACES), so stub it.
// LAN hostname discovery is non-essential; AP/STA web access uses the IP (below).
struct OnePageMdnsStub { bool begin(const char*) { return false; } void end() {} };
static OnePageMdnsStub MDNS;
#else
#include <ESPmDNS.h>
#endif
...
#ifdef ONEPAGE_C61
    // OnePage has no mDNS, so crosspoint.local never resolves — encode the raw
    // AP IP in the QR and show it as the primary URL (always reachable).
    const std::string qrUrl = std::string("http://") + connectedIP + "/";
    const char* primaryText = qrUrl.c_str();
    const char* secondaryText = nullptr;
#else
    const std::string qrUrl = hostnameUrl;
    const char* primaryText = hostnameUrl.c_str();
    const char* secondaryText = ipUrl.c_str();
#endif

    // Show QR code for URL
    const Rect qrBoundsUrl(metrics.contentSidePadding, startY, QR_CODE_WIDTH, QR_CODE_HEIGHT);
    QrUtils::drawQrCode(renderer, qrBoundsUrl, qrUrl);

    // Show URL text next to the QR
    renderer.drawText(UI_10_FONT_ID, metrics.contentSidePadding + QR_CODE_WIDTH + metrics.verticalSpacing, startY + 80,
                      primaryText);
    if (secondaryText != nullptr) {
      renderer.drawText(SMALL_FONT_ID, metrics.contentSidePadding + QR_CODE_WIDTH + metrics.verticalSpacing,
                        startY + 100, secondaryText);
    }
  } else {
