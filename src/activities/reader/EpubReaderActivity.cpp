...
void EpubReaderActivity::drawRuledLines(const Page& page, const int fontId, const int orientedMarginTop,
  ...
  const int offset = static_cast<int>(SETTINGS.ruledLineOffset) - 8;  // bias 8 -> -8..+12px
  ...
  for (const auto& el : page.elements) {
    ...
    switch (style) {
      ...
      case 4: {  // zigzag (folded line): alternating up/down diagonal segments
        const int period = dash + gap;                       // horizontal span of one V leg
